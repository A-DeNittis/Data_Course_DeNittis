...
ggplot(meanvec, aes(x=Age_wk, y=mean))+
  geom_line() +ylab("Mean Area (cm^2)")+xlab("Age (weeks)") + theme_bw()
```

This can be visualized better by plotting growth rate calculated by dividing each mean by the previous observation mean. A value of 1.0 indicates no growth. Values < 1.0 represent a decrease in size, while values > 1.0 represent an increase in size. While a value over 1.0 indicates an increase in size following subculture, a decrease in the interval indicates that there was less overall growth compared to the previous week. This can possibly help determine at what points growth increases the most, or at what point growth stabilizes.

```{r echo=FALSE}
grvec = c(1,1.84,1.51,1.63,.966,1.51,1.15,1.37,1.08,1.02,1.02,0.892)
meanvec = bind_cols(meanvec,gr=grvec)

ggplot(meanvec, aes(x=Age_wk, y=gr))+
  geom_line() + 
  theme_bw() + 
  geom_hline(aes(yintercept = 1.0)) + ylab("Growth Rate") + xlab("Age (Weeks)")
```
...
**ARIMA Modeling (Failures)**

The goal of this project was to apply ARIMA modeling to the dataset. However, I was not able to successfully execute it with my data for this project. Assuming it was an issue with my experimental data, I created a new data set modeled off the experimental data for a clean data set. However, this did not solve the issue. I assume the issue lies in my coding or poor understanding of the concepts. Instead of going through the modeling process with my data, I will go through some of the basic methodology with a preloaded data set. The benefit for ARIMA modeling with my data is real time analysis and forecasting for expected plant growth based on general trends, or cycles in which the mean of my univariate data increases and decreases. This could be particularly useful for determining the ideal times to subculture plantlets to maintain as high of vigor as possible. Since this study focuses primarily on establishing a protocol for mass producing plantlets, determining these critical points in the tissue culture process could produce a more informed protocol.   
