...
glimpse(dat) #<dbl> stores 16 decimal points (can't see in class), takes up far more memory /cn
str(dat)
...
for(i in names(dat)){
  plot(dat[,"mpg"] ~ dat[,i], xlab = i, ylab = "MPG", main = i) # plotting MPG against all variables /cn
}
rm(i)
...
x = seq(0,50,by=1)
y = y <- dbinom(x,50,0.5) #Random gen for binomial distribution
