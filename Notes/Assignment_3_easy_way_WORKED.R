...
##/cn denotes class notes
## file manipulation, "for loop" on other stuff, dplyr verbs /cn

################# Make csv files to test function on ################
setwd("C:/Users/Aly/Desktop/GIT/Data_Course_DeNittis/")
...
dat = read.csv("thatch_ant.csv", stringsAsFactors = FALSE) # does not assign strings as factors /cn
?read.csv
...
dat %>% select(-Colony) %>% head() #Everything but colony, view head without colony column /cn
...
#############################
##Combine dataframes /cn


df1 = data.frame(Col1 = c(1:100),Col2 = c(rep("A", 50), rep("B", 50)))
df2 = data.frame(Col1 = c(1:10),Col2 = c(rep("A", 20), rep("B", 80)))
...
df4 = left_join(df1, df2, by = "Col1")

#############################

df = read.delim("US_Biome_Diversity.tsv", sep = "\t")
#Order from most to least diverse
decrease = order(df$Diversity, decreasing = TRUE)
df2 = df[decrease,]

?sort

library(plyr)
mapvalues(df$Biome, from = "Temperate", to = "Junk")
?mapvalues
